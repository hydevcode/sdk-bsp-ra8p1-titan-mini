...
TCM_ITCM_CODE int16_t decode_output_layer(const float *out_f,
                               int16_t grid,
...
TCM_ITCM_CODE void rgb565_to_gray_resize_192_and_quantization(const uint16_t *src, int16_t src_w, int16_t src_h, int8_t *i8_buf)
{
...
TCM_ITCM_CODE void rgb565_to_gray_resize_192_and_quantization_fast(const uint16_t *src, int16_t src_w, int16_t src_h, int8_t *i8_buf)
{
    const int16_t dst_w = 192;
    const int16_t dst_h = 192;
    uint32_t y_acc = 0;
    uint32_t y_step = ((uint32_t)src_h << 16) / (uint32_t)dst_h;
    uint32_t x_step = ((uint32_t)src_w << 16) / (uint32_t)dst_w;

    for (int16_t y = 0; y < dst_h; y++)
    {
        const uint16_t *row = src + ((y_acc >> 16) * (uint32_t)src_w);
        uint32_t x_acc = 0;

        for (int16_t x = 0; x < dst_w; x++)
        {
            uint16_t pix = row[x_acc >> 16];
            uint32_t r = (pix >> 11) & 0x1FU;
            uint32_t g = (pix >> 5) & 0x3FU;
            uint32_t b = pix & 0x1FU;
            uint32_t gray;
            int32_t qi;
            ...
            i8_buf[(uint32_t)y * (uint32_t)dst_w + (uint32_t)x] = (int8_t)qi;
            x_acc += x_step;
        }
...
TCM_ITCM_CODE void dequantize_int8(const int8_t *in, float *out,
...
TCM_ITCM_CODE float iou_rect(const det_box_t *a, const det_box_t *b)
{
...
TCM_ITCM_CODE float diou_rect(const det_box_t *a, const det_box_t *b)
{
    // IoU
...
TCM_ITCM_CODE void sort_boxes_by_score(det_box_t *boxes, int16_t n)
{
...
TCM_ITCM_CODE int16_t nms_filter(det_box_t *boxes, int16_t n, float iou_thresh)
{
    if (n <= 0) return 0;
